...
# 1. Recuperar sesión (Memory Latching + Cookie fallback)

if not st.session_state.authenticated:

    # 🆕 SINCRONIZACIÓN F5: Damos tiempo al navegador para enviar cookies en el primer arranque
    ...
    if st.session_state.login_successful:
        ...
        st.session_state.login_successful = False # Consumimos solo si entramos

    

    # Prioridad B: Buscar en Cookies (solo si no es un Logout voluntario)

    elif not st.session_state.logout_in_progress:

        auth_data = cookie_manager.get(cookie="perez_login_token")

        if auth_data:

            try:

                c_role, c_name = auth_data.split("|")

                # Solo restauramos si el rol coincide con el portal actual o es zona neutral

                if (is_staff_portal and c_role == "booster") or \

                   (is_admin_portal and c_role == "admin") or \

                   (is_token_view or is_ranking_view):
    ...
    if not st.session_state.authenticated:
        ...
        if k_param:
            ...
            if not df_b.empty:
                ...
                st.session_state.user_name = df_b.iloc[0]['nombre']

                st.session_state.login_successful = True

                cookie_manager.set("perez_login_token", f"booster|{st.session_state.user_name}", expires_at=(datetime.now() + timedelta(days=3)))
...
# 2. Protección de Portal

if st.session_state.authenticated:

    role_ok = False

    if is_token_view or is_ranking_view:

        role_ok = True # Zona neutral, permitida para todos

    elif is_staff_portal and st.session_state.user_role == "booster":

        role_ok = True

    elif is_admin_portal and st.session_state.user_role == "admin": 
...
# Se ejecutan ANTES de la autenticación obligatoria para permitir acceso directo
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""
        ...
        <div style="margin-bottom: 20px;">
            ...
            <a href="/?v=staff" target="_self" style="text-decoration: none;">

                <button style="background-color: #1a1a1a; color: #58a6ff; border: 1px solid #30363d; padding: 8px 15px; border-radius: 6px; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 8px;">

                    ⬅️ Volver al Portal de Staff
                ...
                with st.form("staff_login_form"):
                    ...
                    if submit:
                        ...
                        if not df_check.empty:
                            ...
                            st.session_state.user_name = df_check.iloc[0]['nombre']
                            ...
                            if mantener:
                                ...
                                cookie_manager.set("perez_login_token", f"booster|{st.session_state.user_name}", expires_at=expira)
    ...
    st.markdown(f"""
        ...
        <div style='text-align: center; margin-top: 20px; margin-bottom: 10px;'>
            ...
            <p style='color: #a0a0a0; font-size: 28px; font-weight: 900; text-transform: uppercase; letter-spacing: 5px; margin-top: 20px;'>
                ...
                Temporada de {NOMBRE_MES_ACTUAL} <span style='color: #ff0000; text-shadow: 0 0 10px #ff0000;'>ðŸ”´</span>
...
<div class="global-stats-panel">
    ...
    <div class="stat-segment"><p class="stat-title">📊 WR Global</p><p class="stat-value" style="color: #f1c40f;">{wr_global:.1f}%</p></div>
        ...
        tabla_html = "<table class='esports-table'><thead><tr><th>NÂ°</th><th style='text-align:left'>Staff</th><th>âœ… Term.</th><th>ðŸ”¥ Bonos WR</th><th>âŒ Aban.</th><th>â­ Pts</th></tr></thead><tbody>"

        for index, row in df_rank.iterrows():

            tabla_html += f"<tr><td>{index+1}Â°</td><td class='col-staff'>{row['booster_nombre']}</td><td>{row['terminados']}</td><td>{row['high_wr']}</td><td style='color:#e74c3c'>{row['abandonos']}</td><td style='color:#2ecc71; font-weight:bold;'>{row['puntaje']} pts</td></tr>"
...
def render_booster_dashboard(booster_name):
    ...
    tabs = st.tabs(["ðŸ“ˆ Mi Rendimiento", "âš™ï¸ Ajustes de Perfil"])
    ...
    with tabs[0]:
        ...
        st.subheader("ðŸ› ï¸ Cuentas en Curso")

        if df_activos.empty:

            st.success("Â¡Todo al dÃ­a! No tienes pedidos pendientes.")

        else:

            for _, row in df_activos.iterrows():

                with st.expander(f"ðŸŽ® {row['user_pass']}"):
                    ...
                    with ca:
                        ...
                        st.info(f"ðŸ“ **Notas:** {v_notas}")

                        if row['opgg']: st.write(f"ðŸ”— [Ver OP.GG]({row['opgg']})")
                    ...
                    with cb:
                        ...
                        st.markdown(f"<a href='/?t={token}' target='_self' style='text-decoration:none;'><button style='width:100%; cursor:pointer; background:#2ecc71; border:none; color:black; padding:10px; border-radius:5px; font-weight:bold;'>VER DETALLES</button></a>", unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        st.subheader("âš™ï¸ ConfiguraciÃ³n de Acceso")
...
if st.session_state.authenticated:
    ...
    # Header Uniforme con BotÃ³n de Cerrar SesiÃ³n
    ...
    with h1:
        ...
        if st.session_state.user_role == "admin":
            ...
            st.title(f"ðŸ‘¨â€ðŸ’» Panel Staff: {st.session_state.user_name}")
...
tab_reportes, tab_analytics, tab_inventario, tab_ranking, tab_tracking, tab_gestion, tab_binance = st.tabs(["ðŸ“Š Reportes", "ðŸ“ˆ Analytics", "ðŸ“¦ INVENTARIO", "ðŸ† TOP STAFF", "ðŸ” TRACKING", "ðŸ› ï¸ GESTIÃ“N", "ðŸ’° BINANCE"])
...
with tab_reportes:
    ...
    with f4:
        ...
        if st.button("🚨 Auditoría Alert", use_container_width=True, help="Revisa retrasos y envía pings a Discord"):

            with st.spinner("Auditando pedidos críticos..."):

                count = ejecutar_auditoria_alertas()

                if count and count > 0:

                    st.success(f"Auditado: {count} alertas enviadas a Discord.")

                else:

                    st.info("Todo al día. No se detectaron anomalías.")
    ...
    if not df_rep.empty:
        ...
        m1.metric("ðŸ“¦ Pedidos", f"{conteo}")

        m2.metric("ðŸ’° Mi Neto", f"${t_neto:.2f}")

        m3.metric("ðŸ‘¤ Pago Staff", f"${t_staff:.2f}")

        m4.metric("ðŸ¦ Bote Ranking", f"${t_bote:.2f}")

        m5.metric("ðŸ“ˆ Ventas Totales", f"${t_ventas:.2f}")
...
with tab_analytics:
    ...
    if not df_bi.empty:
        ...
        st.markdown(f"**ðŸŽï¸ Ranking de Velocidad Operativa ({mes_sel_ana})**")

        st.caption("DÃ­as promedio por entrega (Solo pedidos pagados).")
        ...
        fig_bar = px.bar(
            ...
            labels={
                ...
                "Tiempo_Promedio": "DÃ­as Promedio por Entrega",
...
with tab_ranking:
    ...
    st.subheader("ðŸ† Hall of Fame: Valor y Eficiencia")
...
with tab_tracking:
    ...
    with c1:
        ...
        st.subheader("ðŸ” Tracking Operativo")
    ...
    if df_activos.empty:
        ...
        st.subheader("ðŸ› ï¸ GestiÃ³n de Enlaces (CorrecciÃ³n de Errores)")
            ...
            if submit_edit:
                ...
                if conn:
                    ...
                    try:
                        ...
                        st.success(f"âœ… Â¡OperaciÃ³n exitosa! Refrescando...")
                    ...
                    st.error("âŒ Error de conexiÃ³n a la nube.")
    ...
    if not df_audit.empty:
        ...
        for _, row in df_audit.iterrows():
            ...
            if not has_opgg and dias <= -3:
                ...
                alerta_texto = "RETRASO CRÃTICO" if not alerta_texto else f"{alerta_texto} & RETRASO"
...
with tab_gestion:
    ...
    if not df_pendientes.empty:
        ...
        for index, row in df_pendientes.iterrows():
            ...
            titulo_expander = f"ðŸ”´ Pendiente: {row['booster_nombre']} ï½œ Cuenta: {row['user_pass']}"
            ...
            with st.expander(titulo_expander):
                ...
                with st.form(key=f"form_pay_lock_{row['id']}"):
                    ...
                    marcar_pagado = c6.checkbox("CONFIRMAR PAGO", value=False, 
                                                ...
                                                help="Al confirmar, este pedido se sumarÃ¡ a tus mÃ©tricas financieras.")
                        ...
                        if marcar_pagado:
                            ...
                            if conn:
                                ...
                                try:
                                    ...
                                    st.success(f"âœ… Â¡Pago registrado! {row['booster_nombre']} liquidado.")
        ...
        st.success("âœ¨ Â¡Todo al dÃ­a! No tienes pagos pendientes por ahora.")
...
@st.dialog("âœï¸ Modificar TransacciÃ³n")
...
def modal_editar_transaccion(fila):
    ...
    with st.form("form_modal_edit"):
        ...
        e_cat = st.selectbox("CategorÃ­a:", ["NETO", "BOTE"], index=0 if fila['categoria'] == 'NETO' else 1)

        e_monto = st.number_input("Monto ($):", min_value=0.01, step=1.00, value=float(fila['monto']), format="%.2f")

        e_desc = st.text_input("DescripciÃ³n:", value=fila['descripcion'])



        guardar = st.form_submit_button("ðŸ’¾ Guardar Cambios", type="primary", use_container_width=True)
...
def modal_eliminar_transaccion(id_real, detalle):
    ...
    st.error("Â¿EstÃ¡s seguro de eliminar este movimiento?")

    st.write(f"**{detalle}**")

    st.write("Esta acciÃ³n recalcularÃ¡ tu Binance y no se puede deshacer.")

    

    if st.button("ðŸ—‘ï¸ SÃ­, Eliminar Definitivamente", type="primary", use_container_width=True):
        ...
        if conn:
            ...
            try:
                ...
                st.success("ðŸ—‘ï¸ Registro eliminado.")
...
with tab_binance:
    ...
    st.subheader("ðŸ¦ Binance Wallet")



    # 1. CÃLCULO EN VIVO DE FONDOS (Igual al PC)
    ...
    # Ajuste histÃ³rico forzoso (Mismo que en main.py)
        ...
        with st.form("form_wallet"):
            ...
            if st.form_submit_button("Registrar Movimiento", use_container_width=True):
                ...
                if not desc_tx:
                    ...
                    st.error("Por favor agrega una descripciÃ³n.")
                    ...
                    if conn:
                        ...
                        try:
                            ...
                            st.success("âœ… Registrado con Ã©xito.")
    ...
    with col_hist:
        ...
        if df_wallet.empty:
            ...
            with c_filtro:
                ...
                mes_seleccionado = st.selectbox("ðŸ“… Filtrar por mes:", opciones_filtro, index=idx_defecto)
            ...
            if df_filtrado.empty:
                ...
                df_final.columns = ["NÂº", "Fecha", "Tipo", "Caja", "Monto", "Detalle"]
                ...
                st.markdown("### âš™ï¸ Gestionar Registro")

                opciones_crud = df_filtrado.apply(lambda r: f"NÂº {r['id_visual']} | {r['tipo']} | {r['monto_str']} | {r['descripcion']} (ID:{r['id']})", axis=1).tolist()
                ...
                with c_btn1:
                    ...
                    if st.button("âœï¸ Editar", use_container_width=True):
                ...
                with c_btn2:
                    ...
                    if st.button("ðŸ—‘ï¸ Eliminar", use_container_width=True):
